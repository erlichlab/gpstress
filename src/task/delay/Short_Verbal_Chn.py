# Verbal tasks with shorter delays (in seconds) Chinese version
...
def ShortVerbal(var,doo,myPoints,dbc,mouse,getName=True,sub_id=None,pre_points=0,pre_trials=0,pre_trialsCorrect=0):
    ...
    setupList = fc.verbalSetup(var.verbalTotalTrial_Chn,var.mag,var.shortmag) # setup the reward of blocks for the whole experiment(same as in nonverbal)
    shuffledSetupList = copy.copy(setupList)# shuffle the reward list
    random.shuffle(shuffledSetupList)
    fc.stage_instruction_Chn(stageName,var,doo) # show subjects stage name (short delay)
